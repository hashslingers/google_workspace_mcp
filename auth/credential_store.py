...
class LocalDirectoryCredentialStore(CredentialStore):
    ...
    def _get_credential_path(self, user_email: str) -> str:
        ...
        When WORKSPACE_MCP_PORT is set, the port is appended to the filename so
        that multiple server instances (e.g. Sheets on port 8000 and Apps Script
        on port 8006) each maintain their own credential file instead of
        overwriting each other's scopes and refresh tokens.
        """
        ...
        port_suffix = f"_port{port}" if port else ""

        safe_email = quote(user_email, safe="@._-")
        creds_path = self._resolve_credential_path(
            f"{safe_email}{port_suffix}{self.FILE_EXTENSION}"
        )
        ...
        if legacy_safe_email != safe_email:
            legacy_path = self._resolve_credential_path(
                f"{legacy_safe_email}{port_suffix}{self.FILE_EXTENSION}"
            )
